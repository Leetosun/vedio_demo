...
public class GridVideoViewContainerAdapter extends VideoViewAdapter {
    ...
    public void notifyUiChanged(HashMap<Integer, SurfaceView> uids, int localUid, HashMap<Integer, Integer> status,
        ...
        // log.debug("notifyUiChanged " + (mLocalUid & 0xFFFFFFFFL) + " " + (localUid & 0xFFFFFFFFL) + " " + uids + " "
        // + status + " " + volume);
    }
    ...
    public long getItemId(int position) {
        ...
        if (view == null) {
            throw new NullPointerException("SurfaceView destroyed for user " + (user.mUid & 0xFFFFFFFFL) + " "
                    + user.mStatus + " " + user.mVolume);
        }
