package com.lovcreate.teacher.view;
...
import com.lovcreate.teacher.propeller.ui.ViewUtil;
...
public class VideoViewAdapterUtil {
    ...
    public static void composeDataItem1(final ArrayList<UserStatusData> users, HashMap<Integer, SurfaceView> uids,
            int localUid) {
        for (HashMap.Entry<Integer, SurfaceView> entry : uids.entrySet()) {
            if (DEBUG) {
                // log.debug("composeDataItem1 " + (entry.getKey() & 0xFFFFFFFFL) + " " + (localUid & 0xFFFFFFFFL) + " "
                // + users.size() + " " + entry.getValue());
            }
            SurfaceView surfaceV = entry.getValue();
            surfaceV.setZOrderOnTop(false);
            surfaceV.setZOrderMediaOverlay(false);
            searchUidsAndAppend(users, entry, localUid, UserStatusData.DEFAULT_STATUS, UserStatusData.DEFAULT_VOLUME,
                    null);
        }
    ...
    private static void removeNotExisted(ArrayList<UserStatusData> users, HashMap<Integer, SurfaceView> uids,
            int localUid) {
        if (DEBUG) {
    ...
    public static void composeDataItem(final ArrayList<UserStatusData> users, HashMap<Integer, SurfaceView> uids,
            int localUid, HashMap<Integer, Integer> status, HashMap<Integer, Integer> volume,
    ...
    public static void composeDataItem(final ArrayList<UserStatusData> users, HashMap<Integer, SurfaceView> uids,
        ...
        for (HashMap.Entry<Integer, SurfaceView> entry : uids.entrySet()) {
            ...
            if (DEBUG) {
                // log.debug("composeDataItem " + users + " " + entry + " " + (localUid & 0XFFFFFFFFL) + " " + s + " " +
                // v + " " + i + " " + local + " " + (uid & 0XFFFFFFFFL) + " " + (uidExcepted & 0XFFFFFFFFL));
            }
    ...
    public static void renderExtraData(Context context, UserStatusData user, VideoUserStatusHolder myHolder) {
        ...
        if (tag != null && System.currentTimeMillis() - (Long) tag < 1500) { // workaround for audio volume comes just
                                                                             // later than mute
            return;
        }
