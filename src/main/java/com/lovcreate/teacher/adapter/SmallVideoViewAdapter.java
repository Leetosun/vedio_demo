package com.lovcreate.teacher.adapter;
...
import com.lovcreate.teacher.view.VideoViewAdapter;
import com.lovcreate.teacher.view.VideoViewAdapterUtil;
...
public class SmallVideoViewAdapter extends VideoViewAdapter {
    ...
    public SmallVideoViewAdapter(Context context, int localUid, int exceptedUid, HashMap<Integer, SurfaceView> uids,
    ...
    @Override
    public void notifyUiChanged(HashMap<Integer, SurfaceView> uids, int uidExcepted, HashMap<Integer, Integer> status,
            HashMap<Integer, Integer> volume) {
        mUsers.clear();

        mExceptedUid = uidExcepted;

        // log.debug("notifyUiChanged " + (mLocalUid & 0xFFFFFFFFL) + " " + (uidExcepted & 0xFFFFFFFFL) + " " + uids + "
        // " + status + " " + volume);
